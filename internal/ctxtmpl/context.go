...
type scanner struct {
	...
	curURL bool
	// urlPrefix accumulates the whitespace-stripped, lowercased prefix of the
	// current attribute value rendered so far (literal template text plus
	// interpolated values), capped at maxSchemeProbe bytes. Scheme validation
	// must run against this whole prefix rather than a single interpolation
	// value; otherwise "javascript:" split across two interpolations, or
	// across literal text and an interpolation, bypasses the check.
	urlPrefix []byte
}
