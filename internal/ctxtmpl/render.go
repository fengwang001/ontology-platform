...
func Render(tmpl string, data map[string]string) (string, error) {
	...
	for {
		...
		if ctx.urlAttr {
			// 根因修复：旧代码只把当前这一次插值的取值交给
			// checkDangerousURL，攻击者把 "javascript:" 拆进两次插值
			// （或一半写死在模板字面量里）即可绕过。现在判定基于该
			// URL 属性值已渲染出的完整前缀（urlValuePrefix 累积了字面
			// 量与此前所有插值）拼上本次取值；非起始位置的值因为前缀
			// 非空且不以危险协议开头，天然放行，不会误伤。
			if err := checkDangerousURL(s.urlValuePrefix() + value); err != nil {
				return "", err
			}
		}
